...
//rolling ball w/ negative acceleration and velocity
typedef struct {
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
...
//one of the 6 holes in the table
typedef struct {
    phylib_coord pos;
} phylib_hole;

//a horizontal cushion
typedef struct {
    double y;
} phylib_hcushion;
...
//c union which stores any/all of the above classes/structures in the same space
typedef union {
...
//union annot identify the class of the object, so we need another structure for that
//type is the enum indicating the class of the object, and obj is the object itself
typedef struct {
    phylib_obj type;
    phylib_untyped obj;
} phylib_object;
...
void phylib_copy_object( phylib_object **dest, phylib_object **src ); //complete
